...
versions = 2;
versionCB = mod(pptNo,versions)+1;
...
numberOfTrials = {40}; %enter the number of trials (games) you want for each block
numberOfTargets = {[4 6 8 10]}; %enter the number of targets per trial you want for each block
...
if versionCB ==1
    condition = [repmat(1,[1,numel(IDList)]),repmat([2,3],[1,numel(IDList)/versions])];
elseif versionCB ==2
    condition = [repmat(1,[1,numel(IDList)]),repmat([2,3],[1,numel(IDList)/versions])];
end
...
for block = blockList
    ...
    for trial = 1:numberOfTrials{block}
            ...
            id = 1;
                    ...
                    for n = 1:imagesShown
                        for m = 1:targetsPerIdentity
                            ...
                            allTargetImages{end+1} = targetImages{n,m};
            ...
            identityName{right} = nameList{1}{whichImage*2};
                        
            combinedImages = {allTargetImages{(imagesShown+1):(trialTargets+imagesShown)},foilImages{1:numberOfFoils}};
        ...
        for n = 1:imagesShown
            targetTexture(end+1) = Screen('MakeTexture', window, allTargetImages{n});
        end
        ...
        if imagesShown > 1
            ...
            if shuffledCondition{block}(trial) == 2
                ...
                DrawFormattedText(window, identityName{left}, mean([targetArrayPosition(1,left), targetArrayPosition(3,left)])-textOffset, YposSubtext, white);
                DrawFormattedText(window, identityName{left}, mean([targetArrayPosition(1,right), targetArrayPosition(3,right)])-textOffset, YposSubtext, white);

            elseif shuffledCondition{block}(trial) == 3
                DrawFormattedText(window, targetTextDifferent, XposHeading, YposHeading, white);
                DrawFormattedText(window, identityName{left}, mean([targetArrayPosition(1,left), targetArrayPosition(3,left)])-textOffset, YposSubtext, white);
                DrawFormattedText(window, identityName{right}, mean([targetArrayPosition(1,right), targetArrayPosition(3,right)])-textOffset, YposSubtext, white);
            end
            ...
            while ~any(buttons)
                ...
                if keyIsDown
                    ...
                    if keyCode(nextKey); endTrial = 1;
                        RTstart = toc(tstart);
                        Screen('Flip',window);          
                        DrawFormattedText(window, 'End Trial', 'center', 'center', white);
                        Screen('Flip',window);
                        break;
                    end
                end
    ...
    %                             shuffledImageNumberDisplay(clickedPositionIndex) = [];
                    ...
                    elseif selfTerminate{block}==0 && targetsRemaining == 0;%if there are no faces left
                        Screen('Flip',window);          
                        DrawFormattedText(window, 'Complete!', 'center', 'center', white);
                        Screen('Flip',window);          
                        WaitSecs(ITI);
                        endTrial = 1;
                    elseif endTrial == 0; 
                    ...
                    if clickedPositionIndex ~= 0;                        
                        ...
                        pos=pos+1;  DATAmatrix(experimentClick,pos) = targetNumber;                                 heading{pos} = 'Target Number';
        ...
        if endTrial == 1
            ...
            pos=pos+1; summaryMatrix(experimentTrial,pos) = targetNumber;                               summaryHeading{pos} = 'Target Number';
